...
private fun checkMethodSignature(
    ...
    val omittedOptionalParams = nodeParams.size < baseParams.size
            && nodeParams.indices.all { i ->
                val nodeParamType = nodeParams[i].type
                val baseParamType = baseParams[i].type

                if (nodeParamType == null || baseParamType == null) return@all true

                if (isClassTypeParameterReference(baseParamType, baseDeclaration)) return@all true

                typeChecker.typeToString(typeChecker.getTypeFromTypeNode(nodeParamType)) ==
                        typeChecker.typeToString(typeChecker.getTypeFromTypeNode(baseParamType))
            }
            && ((nodeParams.size until baseParams.size).all { i ->
                baseParams[i].questionToken != null || baseParams[i].initializer != null
            })

    if (!omittedOptionalParams && nodeParams.size != baseParams.size) return SignatureCheckResult.Incompatible
